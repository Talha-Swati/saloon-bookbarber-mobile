...
import { fetchAssignedBookings, ProfessionalBooking } from "@/services/professionalService";
...
export default function ProfessionalBookings() {
  ...
  useFocusEffect(useCallback(() => {
    ...
    setLoading(true);
    fetchAssignedBookings().then(setItems).finally(() => setLoading(false));
  }, [role]));
  ...
  return <Screen><Header /><View style={s.tabs}>{(["today", "upcoming", "completed"] as Filter[]).map((filter) => <Pressable key={filter} onPress={() => setActive(filter)} style={[s.tab, active === filter && s.tabOn]}><Text style={[s.tabText, active === filter && s.tabTextOn]}>{filter[0].toUpperCase() + filter.slice(1)}</Text></Pressable>)}</View>{loading ? <ActivityIndicator color={colors.primary} /> : filtered.length ? filtered.map((item) => <Pressable key={item.id} style={s.card} onPress={() => router.push({ pathname: "/professional/booking/[id]", params: { id: item.id } })}><View style={s.top}><Text style={s.name}>{item.customerName}</Text><Status status={item.status} /></View><Text style={s.service}>{item.serviceName}</Text><Text style={s.meta}>{item.date === "today" ? "Today" : item.date} · {item.time} · {item.durationMinutes} min</Text></Pressable>) : <Text style={s.empty}>No assigned bookings in this view.</Text>}</Screen>;
}
